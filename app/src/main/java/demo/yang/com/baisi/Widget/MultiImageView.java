...
import demo.yang.com.baisi.Utils.ColorFilterImageView;
import demo.yang.com.baisi.Utils.DensityUtil;
...
public class MultiImageView extends LinearLayout {
    ...
    /** 长度 单位为Pixel **/
    private int pxOneMaxWandH;  // 单张图最大允许宽度
    private int twoMaxWidth=0;
    private int pxMoreWandH = 0;// 多张图的
...
//            pxOneMaxWandH = 2*(MAX_WIDTH  / 3);//一张图最大宽度
    ...
    private ImageView createImageView(int position, final boolean isMultiImage, int size) {
        ...
        if (IsGIF(url)){
            Glide.with(getContext()).load(url).asGif().placeholder(R.drawable.yaoming).error(R.drawable.b).diskCacheStrategy(DiskCacheStrategy.SOURCE).dontAnimate().into(imageView);
        }else {
            Glide.with(getContext()).load(url).thumbnail(0.1f).placeholder(R.drawable.yaoming).error(R.drawable.b).diskCacheStrategy(DiskCacheStrategy.ALL).dontAnimate().into(imageView);
        }
        return imageView;
    }
